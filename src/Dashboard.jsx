...
function rankOpacity(index) {
  if (index === 0) return 1;
  if (index === 1) return 0.9;
  if (index === 2) return 0.8;
  return Math.max(0.28, 0.66 - (index - 3) * 0.065);
}

function RankChart({ rows, dataKey, color, name, columns = false }) {
  const data = [...rows].sort((a, b) => b[dataKey] - a[dataKey]).slice(0, 10).map((row) => ({ ...row, label: short(row.name) }));
  if (!data.length) return <Empty />;
  if (columns) {
    return <div className="chart rank-chart rank-columns"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ left: 4, right: 12, top: 12, bottom: 12 }}><CartesianGrid vertical={false} strokeDasharray="4 4" /><XAxis type="category" dataKey="label" interval={0} angle={-28} textAnchor="end" height={82} tick={{ fontSize: 10 }} /><YAxis type="number" allowDecimals={false} width={30} /><Tooltip /><Bar dataKey={dataKey} name={name} fill={color} radius={[6, 6, 0, 0]} maxBarSize={56}>{data.map((item, index) => <Cell key={`${item.name}-${index}`} fill={color} fillOpacity={rankOpacity(index)} />)}<LabelList dataKey={dataKey} position="insideTop" fill="#fff" fontSize={11} fontWeight={800} formatter={visibleValue} /></Bar></BarChart></ResponsiveContainer></div>;
  }
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <section className="grid">
        ...
        <Card title="Ranking de analistas por visitas" subtitle="Responsáveis com maior número de visitas."><RankChart rows={metrics?.analysts || []} dataKey="visits" name="Visitas" color="#246dcc" columns /></Card>
        <Card title="Ranking de analistas por equipamentos" subtitle="Responsáveis com maior volume de equipamentos."><RankChart rows={metrics?.analysts || []} dataKey="equipment" name="Equipamentos" color="#24a36a" columns /></Card>
